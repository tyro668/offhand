...
- 保持架构优雅：长期规则、会话记忆、外部上下文、STT 提示、STT 后纠错分层清晰
...
6. 外部资料导入的归属层不清晰
   - `.md` 导入如果直接落到词典，会把“长期规则”和“外部参考上下文”混在一起
   - 很多 Markdown 信息更适合作为 context reference，而不是 correction/preserve 规则
...
6. 支持冷启动
   - 除了“边用边学”，还要支持从外部资料提前导入上下文
   - 外部导入默认进入独立 context 层，而不是直接混入长期词典
...
但还需要补一层与词典平行的独立外部上下文层：

- `context`
  - 来源于外部资料导入
  - 主要用于 STT 前提示、术语召回、AI 增强参考
  - 默认不直接生成全局纠错规则

建议将其单独建模为：

- `TermContext`
- 或 `ImportedTermContext`
...
4. 场景标签
   ...
   - 招聘/技术/产品等
5. 外部 context 命中
   - 当前上下文与导入 Markdown 的标题、列表项、标签、文件名的匹配度
...
不要把外部 Markdown 导入入口放在这里，避免把“规则管理”和“context 管理”混在一起。

#### Context 页

建议新增独立页面，专门管理外部导入的上下文：

- context 条目列表
- 来源文件
- 当前是否启用
- 是否可提升为词典规则
- 一键提升为 correction / preserve
- 删除或批量禁用
...
这是对“冷启动”和“团队已有知识资产复用”非常重要的一块，但它不应与词典管理混成一层。

目标：

- 允许用户从外部导入一个或多个 `.md` 文件
- 从已有产品文档、会议纪要、术语表、品牌资料中批量提取上下文信息
- 在首次使用前就把高价值 context 预热进系统
...
但这些资料里的内容并不都适合直接成为词典规则：

- 有些适合做 correction/preserve
- 有些只适合做识别参考
- 有些只适合在特定项目、客户、会议下生效

所以更合理的设计不是“Markdown 导入进词典”，而是：

- Markdown 导入进独立 `context` 层
- `TermRecallService` 决定本次识别要不要使用这些 context
- 用户再从 context 中选择是否提升为词典规则

#### 导入后的目标落点

导入结果默认进入独立 context 存储，而不是直接落入词典：

- `context-entry`
  - 独立上下文条目
  - 仅参与召回、prompt、增强参考
- `promotable`
  - 结构清晰、置信度高，可一键提升为词典规则
- `reference-only`
  - 只作为补充参考，不参与规则提升
- `rejected/skipped`
  - 噪声、冲突项或过长正文片段
...
1. 明确的“错误写法 -> 正确写法”映射
   - 默认进入 `context-entry`
   - 仅标记 `promotableAsCorrection`
2. 明确的标准术语、人名、产品名、英文缩写
   - 进入 `context-entry`
   - 标记 `promotableAsPreserve`
3. 仅在正文中高频出现、但没有清晰映射关系的词
   - 只进入 `context-entry` 或 `reference-only`
   - 不直接进入 pending/词典
...
这类内容适合导入为 `preserveHint` 类型的 context。
...
这类内容适合导入为 `correctionHint` 类型的 context。
...
这类内容不一定能直接生成 correction，但很适合抽取 `reference` 类型的 context。
...
3. 分层落库
   - 全部先进入 context
   - 可明确提升为规则的只打 promotion 标记
   - 不够稳定的保持 context-only/reference-only

#### 建议新增 context 类型

不建议把导入来源建模成 `DictionaryEntrySource`，因为 `.md` 导入与词典不是同一个层级。

建议新增独立模型，例如：

- `ImportedContextSource.markdown`
- `TermContextEntryType.correctionHint`
- `TermContextEntryType.preserveHint`
- `TermContextEntryType.reference`

同时建议 `TermLearningEvent.sourceType` 增加：

- `markdown_context_import`
...
导入能力一定要保守，否则很容易污染识别上下文，甚至误伤长期规则。
...
2. 若导入 correction 的 `original` 已存在但 `corrected` 不同
   - 不直接覆盖
   - context 条目标记“与现有规则冲突”
   - 不自动提升为规则

3. 若导入 preserve 与现有 correction 冲突
   - 默认保留现有 correction
   - preserve 仅保留在 context 层
...
建议不要放在词典页，而是新增独立的 context 管理入口，例如：
...
目标：让团队已有的 Markdown 资料直接转化为可召回的 context 资产，降低冷启动成本。

内容：

1. 新增 `MarkdownTermImportService`
2. 新增 `TermContextService`
3. 支持导入单个或多个 `.md`
4. 支持列表、表格、显式映射的解析
5. 导入结果按 context / promotable / reference-only 分层
6. 新增独立 context 管理入口与结果预览
7. 与 `TermRecallService` 打通，让导入 context 参与后续识别
...
class MarkdownTermImportResult {
  ...
  final List<TermPromotionCandidate> promotableCandidates;
...
- `lib/models/term_context_entry.dart`
...
class TermContextEntry {
  ...
  final String sourceType; // markdown
  final String entryType; // correctionHint / preserveHint / reference
...
- `TermRecallService`
  - 对 accepted/session/context 记忆做相关性召回
- `TermPromptBuilder`
  - 把召回结果拼装成 STT prompt 与纠错参考
- `MarkdownTermImportService`
  - 解析外部 `.md`
  - 抽取术语与映射
  - 生成 context / promotable / reference-only 三类结果
- `TermContextService`
  - 持久化和管理外部 context 条目
  - 支持启用/禁用/删除/提升为词典规则
...
目标：支持从团队已有文档中批量预热 context 系统，而不是直接污染词典。
...
3. 支持 `.md` 文件选择与解析预览
4. 支持列表、表格、显式映射解析
5. 支持导入到 context / promotable / reference-only
6. 与 `TermRecallService` 打通，让导入 context 参与后续识别
7. 支持从 context 一键提升为词典规则
...
### 风险 5：Markdown 文档正文噪声太大，导入后污染识别上下文

应对：

- 结构化内容优先，正文抽取保守
- 明确映射也默认先进 context，再由用户提升为规则
- 普通正文高频词默认先进 context-only 或 reference-only
- 导入前展示预览，导入后允许批量回滚

## 8. 结论

最优雅、也最贴合当前代码的路线，不是重写识别系统，而是在现有架构上补齐四层能力：
...
4. `TermContextService` + `MarkdownTermImportService`
   - 负责把外部 `.md` 文档转成可复用的上下文资产
...
- 团队已有 Markdown 文档也能直接转成上下文资产，解决冷启动
- 会议场景几乎零额外架构成本就能获得同样收益

## 9. 本仓库建议的首批实施点

如果按投入产出比排序，我建议先做这 7 个点：
...
7. 增加独立 context 管理入口，支持 Markdown 预热导入

这 7 个点完成后，产品体验上就已经能明显感受到：

- 改过一次的专业术语，下一次更容易对
- 同一次会议里术语会越来越稳定
- 词典不再只是“事后纠错”，而开始参与“事前识别”
- 新项目可以先导入 `.md` context 资料，再开始用，避免冷启动阶段识别很差
